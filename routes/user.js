...
router.post('/register', function (req, res, next) {
  UserController.register(req.body, function (err, data, isAlreadyExist) {
    if(err)
      res.status(500).send({message:"Error registering user. Please try again later.", statusCode : 500});
    else if(isAlreadyExist)
      res.status(201).send({message:"User with given email Id already exist. Please try with another email.", statusCode : 201});
    else
